...
mod tests {
    ...
    fn retrieval_respects_k_limit() -> Result<()> {
        ...
        assert_eq!(top_k(storage.as_ref(), &query, 0)?.len(), 0);
        assert_eq!(top_k(storage.as_ref(), &query, 2)?.len(), 2);
        assert_eq!(top_k(storage.as_ref(), &query, 10)?.len(), 5); // Can't return more than exist
